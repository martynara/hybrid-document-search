...
class QueryEvaluator:
    ...
    def __init__(self, search_advanced_service: AdvancedSearchService):
        """Initialize with search service"""
        self.search_advanced_service = search_advanced_service
    ...
    async def evaluate_queries(self, 
        ...
        for pair in query_chunk_pairs:
            ...
            #search_results = await self.search_service.search_semantic(query=query, limit=top_k)
            search_results = await self.search_advanced_service.multi_collection_search(
...
async def main():
    ...
    limit = 10
    top_k = 20  # Number of results to retrieve for each query
    top_a = 5   # Number of top results to use for metric calculation
    ...
    # Create evaluator
    evaluator = QueryEvaluator(search_advanced_service)
