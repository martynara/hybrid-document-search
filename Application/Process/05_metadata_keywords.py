...
async def main():
    ...
    for i, json_file in enumerate(json_files):
        ...
        for document in chunked_documents.documents:
            document = await keywords_pipeline.process_document(document)
            logger.info(f"Successfully preprocessed keywords for document {document.document_id}")

        chunked_documents.save_to_file(json_file_path)
        logger.info(f"Successfully preprocessed keywords for {json_file}")
